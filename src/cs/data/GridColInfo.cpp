...
const vector<string> GridColInfo::diff_words() const {
	const vector<string> ret = _diffWords;
	return ret;
}

size_t GridColInfo::diff_count() const {
	return _diffWords.size();
}

long int GridColInfo::diff_idx(string& val) const {
	return index_of(_diffWords, val);
}
