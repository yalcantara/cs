...
vector<string> GridInfo::get_column_values(vector<string> data, size_t cols, size_t col) {
...
void GridInfo::fill(vector<string>& data) {
	...
	for (size_t j = 0; j < _cols; j++) {
		vector<string> vals = get_column_values(data, _cols, j);
		colInfo[j].fill(vals);
	}
}
...
void GridInfo::fill(vector<string>& data, size_t col) {
	...
	vector<string> vals = get_column_values(data, _cols, col);
...
size_t GridInfo::diff_count(size_t col) const {
	return colInfo[col].diff_count();
}
const vector<string> GridInfo::diff_words(size_t col) const {
	return colInfo[col].diff_words();
}

long int GridInfo::diff_idx(size_t col, string& word) const {
	return colInfo[col].diff_idx(word);
}

bool GridInfo::has_missing(size_t col) const {
	if (rows() == 0)
		return false;
...
bool GridInfo::is_word(size_t col) const {
	if (rows() == 0)
		return false;
...
bool GridInfo::is_integer(size_t col) const {
	if (rows() == 0)
		return false;
...
bool GridInfo::is_numeric(size_t col) const {
	if (rows() == 0)
		return false;
...
bool GridInfo::is_float(size_t col) const {
	if (rows() == 0)
		return false;
...
bool GridInfo::is_boolean(size_t col) const {
	if (rows() == 0)
		return false;
