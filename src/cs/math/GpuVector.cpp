...
void GpuVector::check_same_length(const GpuVector& b) const {
	if (b.length != length) {
		throw Exception(
				"The length must be the same. Expected " + to_string(length) + ", but got: " + to_string(b.length)
						+ " instead.");
	}
}
...
void GpuVector::copy(Vector& dest) const {
	copy(gpu_cast(dest));
}

void GpuVector::copy(GpuVector& dest) const {
	check_same_length(dest);
	copy_gpu_to_gpu(devPtr, dest.devPtr, length);
}
